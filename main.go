...
func getEnvFloat64(key string, defaultVal float64) float64 {
	if envVal, ok := os.LookupEnv(key); ok {
		envFloat64, err := strconv.ParseFloat(envVal, 64)
		if err == nil {
			return envFloat64
		}
	}
...
func main() {
	var (
		...
		checkKeyGroupsAsyncMinInterval       = flag.String("check-key-groups-async-min-interval", getEnv("REDIS_EXPORTER_CHECK_KEY_GROUPS_MIN_INTERVAL", "60s"), "The min interval between asynchronous key group checks")
		checkKeyGroupsAsyncMaxInterval       = flag.String("check-key-groups-async-max-interval", getEnv("REDIS_EXPORTER_CHECK_KEY_GROUPS_MAX_INTERVAL", "1h"), "The max interval between asynchronous key group checks")
		...
		checkKeyGroupsAsyncTargetUtilization = flag.Float64("check-key-groups-async-target-utilization", getEnvFloat64("REDIS_EXPORTER_CHECK_KEY_GROUPS_ASYNC_TARGET_UTILIZATION", 1), "Target percent utilization of server resources for asynchronous key group scrape")
		...
		checkKeyGroupsAsync                  = flag.Bool("check-key-groups-async", getEnvBool("REDIS_EXPORTER_CHECK_KEY_GROUPS_ASYNC", false), "Whether to check key groups asynchronous rather than synchronously on every scrape")
	...
	minKeyGroupInterval, err := time.ParseDuration(*checkKeyGroupsAsyncMinInterval)
	if err != nil {
		log.Fatalf("Couldn't parse asychronous key group check min interval, err: %s", err)
	}

	maxKeyGroupInterval, err := time.ParseDuration(*checkKeyGroupsAsyncMaxInterval)
	if err != nil {
		log.Fatalf("Couldn't parse asychronous key group check max interval, err: %s", err)
	}
	...
	exp, err := NewRedisExporter(
		*redisAddr,
		Options{
			...
			CheckKeyGroupsAsync:                  *checkKeyGroupsAsync,
			CheckKeyGroupsAsyncMinInterval:       minKeyGroupInterval,
			CheckKeyGroupsAsyncMaxInterval:       maxKeyGroupInterval,
			CheckKeyGroupsAsyncTargetUtilization: *checkKeyGroupsAsyncTargetUtilization,
